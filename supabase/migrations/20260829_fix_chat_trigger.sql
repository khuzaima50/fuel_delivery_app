-- Migration: Fix Chat Notification Trigger (Customer → Driver only)
-- Date: 2026-08-29
-- 
-- Fixes: Skips notifications if profiles.full_name is null. We now check existence in the profiles table.
...
DECLARE
    ...
    payload JSONB;
BEGIN
    -- 1. Verify if the sender is a CUSTOMER (must exist in profiles table)
    -- Drivers are stored in public.drivers table and signup trigger handles them.
    IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = NEW.sender_id) THEN
        -- Sender is not a customer (sender is a driver) -> skip, as Flutter already sent push notification directly
        RETURN NEW;
    END IF;

    -- Get customer full name if set
